...
<head>
	...
	jQuery(function($){
					...
					$(data).each(function () {
						...
						arr.push(
							'<tr trid="',this.id,'">\
								...
								<td>\
									<a href="###" unrel="',this.id,'" data-toggle="modal" data-target="#unbundModal" style="text-decoration: none;">\
									<span class="glyphicon glyphicon-remove"></span>解除关联</a>\
								</td>\
							</tr>'
						)
					});
		...
		$("#dataBody").on("click", "a[unrel]", function () {
			unRelId = $(this).attr("unrel");
		})

		let unRelId;
		$("#unRelBtn").click(function () {
			$.ajax({
				url: "/clue/unRel.do?id="+unRelId,
				type: "delete",
				success(data) {
					if (data.success) {
						closeModal();
						loadRelAct();
						//$("tr:has(a[unrel="+unRelId+"])").remove();
						//$("tr[trid="+unRelId+"]").remove();
					}
					if(data.msg) {
						alert(data.msg);
					}
		...
		$("#relActBtn").click(function () {
			$(":checkbox[name=id]").prop("checked", function () {
				return relArr.includes(this.value);
			});
		})
		...
		$("#relBtn").click(function () {
			$.ajax({
				...
				data: {
					clueId: "${param.id}",
					actIds: $(":checkbox[name=id]:checked").map(function () {
						return this.value;
					}).get().join(",")
				},
				success(data) {
		...
		$("#keyword").keydown(function (e) {
			...
			if(e.which == 13) {
				...
				// 筛选包含关键字的行
				filter(function () {
					return $(this).find("td:eq(1):contains("+k+")").size() > 0;
				}).show();
			}*/
		}).on("input", function () {
			let k = this.value;
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 30%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button id="unRelBtn" type="button" class="btn btn-danger">确定</button>
				</div>
			</div>
		</div>
	</div>
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<div class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input id="keyword" type="text" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default"><span class="glyphicon glyphicon-envelope"></span>
				<a href="mailto:150552288@qq.com">发送邮件</a>
				<a href="tel:18105185234">打电话</a>
			</button>
			<button type="button" class="btn btn-default" onclick="window.location.href='convert.html?id=${param.id}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a id="relActBtn" href="###" data-toggle="modal" data-target="#bundModal" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
