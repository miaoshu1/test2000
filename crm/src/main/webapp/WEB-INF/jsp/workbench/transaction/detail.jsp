...
<script type="text/javascript">
	...
	jQuery(function($){
		...
		let currStage = "09因竞争丢失关闭";

		/*
			绿圈：	glyphicon mystage glyphicon-ok-circle green
			水滴：	glyphicon mystage glyphicon-map-marker green
			黑圈：	glyphicon mystage glyphicon-record
			黑X:  	glyphicon mystage glyphicon-remove
			红X:  	glyphicon mystage glyphicon-remove red
		 */

		$.ajax("/tran/poss.json").done(function (stage2poss) {
			let currPoss = stage2poss[currStage];
			...
			$.ajax({
				...
				success(data) {
					...
					$(data).each(function () {
						...
						if (stage2poss[this.text] != 0) {
							if (stage2poss[this.text] < +currPoss) {
								icon = "glyphicon mystage glyphicon-ok-circle green";
							} else if(stage2poss[this.text] == +currPoss) {
								icon = "glyphicon mystage glyphicon-map-marker green";
							} else {
								icon = "glyphicon mystage glyphicon-record";
							}
						} else {
							if (currStage == this.text) {
								icon = "glyphicon mystage glyphicon-remove red";
							} else {
								icon = "glyphicon mystage glyphicon-remove";
							}
						}

						$("#stages").append('<span class="'+icon+'" data-toggle="popover" ' +
								'data-placement="bottom" data-content="'+this.text+'"></span>-----------')
					})
